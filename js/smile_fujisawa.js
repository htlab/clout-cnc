...
window.onload = function() {
    ...
    soxEventListener.sensorDataReceived = function(soxEvent){
        ...
        if (device.name == "気象警報・注意報藤沢市") {
            var effectiveHumidity = 0;
            var minimumHumidity = 0;

            var statement = device.transducers[6].sensorData.rawValue;
        ...
        if (device.name == "湘南・江ノ島の海の天気") {
            weather = device.transducers[5].sensorData.rawValue;
        ...
        if (device.name == "湘南・江ノ島今日の降水確率2") {
            var percentage = 0;
            var date = new Date();
            var hour = date.getHours();
            var id = Math.floor(hour / 6);
